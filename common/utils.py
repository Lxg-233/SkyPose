...
def getbonelength2(seq, boneindex):
    # Input shape: (B, J, C)
    bone = []
    for index in boneindex:
        bone.append(seq[:, index[1]] - seq[:, index[0]])  # Compute bone vector
    bone = torch.stack(bone, dim=1)  # Stack along the bone dimension
    bone = torch.pow(torch.pow(bone, 2).sum(dim=2), 0.5).unsqueeze(-1)  # Compute bone length
    return bone  # Output shape: (B, num_bones, 1)
