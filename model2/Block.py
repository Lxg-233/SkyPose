...
class IPC(nn.Module):  # c2,gbi==0.02
    ...
    def forward(self, x_gcn, x_conv, bone_features=None):
        ...
        if bone_features is not None:
            ...
            symmetric_pairs = [
                ([1, 2, 3], [4, 5, 6]),  # ÓÒÍÈ-×óÍÈ
                ([11, 12, 13], [14, 15, 16])  # ÓÒ±Û-×ó±Û
            ]

            x_conv_enhanced = x_conv.clone()

            for limb1, limb2 in symmetric_pairs:
                # »ñÈ¡Á½¸ö¶Ô³ÆÖ«ÌåµÄÌØÕ÷
                valid_indices1 = [i for i in limb1 if i < N]
                valid_indices2 = [i for i in limb2 if i < N]

                if valid_indices1 and valid_indices2:
                    limb1_feat = x_conv[:, valid_indices1, :].mean(dim=1)  # [B, C]
                    limb2_feat = x_conv[:, valid_indices2, :].mean(dim=1)  # [B, C]

                    sym_residual = (limb1_feat - limb2_feat).detach() * 0.01
                    for idx in valid_indices1:
                        x_conv_enhanced[:, idx, :] = x_conv[:, idx, :] - sym_residual
                    for idx in valid_indices2:
                        x_conv_enhanced[:, idx, :] = x_conv[:, idx, :] + sym_residual
            x_conv = x_conv_enhanced
